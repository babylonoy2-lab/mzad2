...
export default function Welcome() {
  ...
  return (
    <Screen>
      ...
      <Card>
        <Text style={{ color: theme.text, fontSize: 20, fontWeight: '800', textAlign: 'right' }}>
          كيف تعمل المنصة؟
        </Text>
        {steps.map(([n, t, b]) => (
          <View key={n} style={{ flexDirection: 'row-reverse', gap: 12 }}>
            ...
            >
              <Text style={{ color: theme.background, fontWeight: '900' }}>{n}</Text>
            </View>
            <View style={{ flex: 1 }}>
              <Text style={{ color: theme.text, fontWeight: '800', textAlign: 'right' }}>{t}</Text>
